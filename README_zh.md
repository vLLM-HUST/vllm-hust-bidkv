...
固定源码上的 CPU 契约与真实 scheduler 路径单测已通过。Qwen3.8-27B TP4 graph
真实运行已经证明策略初始化、非零调用、取消后的精确输出恢复和基线回滚；但首次刻意
超配 KV 的压力负载暴露了 victim 轮转问题，因此新增了上述前向进展保护，仍需重新
完成可收敛压力验证。当前只能标记 **unverified**，不能标记 `compatible`。
