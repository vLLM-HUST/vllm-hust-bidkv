...
\section{Conclusion}
BidKV is a request-level victim selector, not a KV compressor. Controlled Ascend TP4 runs establish functional integration, while the repeated interactive cell rejects a default-benefit claim for the current formula and the ascending-mixed cell remains inconclusive. The next contribution gate is a matched serving regime with consistent treatment exposure that proves correct native preempt/recompute, calibrates the surrogate, beats the strongest deployable victim baseline, and explains both beneficial and negative pressure regimes.
